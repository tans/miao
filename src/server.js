  ...
  try {
    ...
    switch (call.name) {
      ...
      case 'app.publish': {
        ...
        const update = await c('apps').updateOne({ id: record.id, draft_version: record.draft_version }, { $set: publishSnapshot(record, timestamp) }); if (!update.modifiedCount) throw new Error('草稿已被其他 Builder 更新，请重新读取后发布');
